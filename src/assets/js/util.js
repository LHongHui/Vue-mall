...
/**
 *  通过接口的每个商品的id 去找对应的addGoods 中数量
 * @param addGoods 本地存储中的数据
 * @param 接口中的一个商品id
 */
const getCartNumById = (addGoods, id) => {
    for (var k in addGoods) {
        if (addGoods[k].id == id) {
            return addGoods[k].num;
        }
    }
}
// 拆分商品 获得多个对应的索引
const getaddGoodsByGoodsId = (addGoods, id) => {
    var indexs = [];
    for (var k in addGoods) {
        if (addGoods[k].id == id) {
            indexs.push(k);
...
/**
 *  通过接口的每个商品的id 去找对应的addGoods 中索引index
 * @param addGoods 本地存储中的数据
 * @param 接口中的一个商品id
 */
const getCartIndexById = (addGoods, shop) => {
    for (var k in addGoods) {
        if (addGoods[k].id == shop.id && addGoods[k].color == shop.color && addGoods[k].size == shop.size) {
            return k;
        }
    }
}
...
export {
    ...
    shuffle,            //数组随机打乱(洗牌函数)
    keyWord,            //搜索关键词高亮显示
    getCartNumById,    // 通过接口的每个商品的id 去找对应的addGoods 中数量
    getCartIndexById,
    getaddGoodsByGoodsId,
