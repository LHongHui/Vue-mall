  ...
  <van-popup v-model="show" position="right" class="popup" :overlay="false">
      ...
      <div id="setAvatar" class="set-avatar border-bottom">
        ...
        <div>
          <img
            v-if="userName"
            :onerror="defaultImg"
            :src="tempAvatar || url + userName.avatar"
            alt
            srcset
          />
          <van-icon name="arrow" />
    ...
    <div class="normal">
      ...
      <van-button
        @click="show = false"
        size="normal"
        class="normal-btn normal-btn-default"
        >取消</van-button
      >
    </div>
  </van-popup>
...
import { goBack, vuexData } from "@/assets/js/mixin";
...
export default {
  ...
  methods: {
    ...
    noName() {
      this.$toast({ message: "用户名暂时不能修改哟~~", className: "zindex" });
    },
    // 修改用户资料
    async setting() {
      try {
        const { data } = await this.Api.user(this.userName._id);
        if (data.code == 200) {
