<template>
  <div class="address">
    ...
    <Scroll v-show="!showFlag">
      ...
      <div v-if="!list || !list.length" class="nolist">暂无收货地址~~</div>
    </Scroll>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    onEdit(item, index) {
      this.setAddress(item); // vuex 存储  addressInfo 并且本地存储了'Address'键值
      this.$router.push("/AddressEdit");
    },
  ...
  async created() {
    ...
    try {
      this.showFlag = true;
      const { data } = await this.Api.getAddress(this.userName._id);
      if (data.success) {
        this.showFlag = false;
